// Cloudflare Pages Function: GET /api/faqs
// 환경 변수는 functions/api/submit.js와 동일하게 SUPABASE_URL, SUPABASE_SERVICE_KEY 사용.
// Supabase 대시보드 > Table Editor > faqs 에서 행을 추가/수정하면 이 API에 바로 반영됨.

export async function onRequestGet({ env }) {
  if (!env.SUPABASE_URL || !env.SUPABASE_SERVICE_KEY) return json([]);

  const res = await fetch(
    `${env.SUPABASE_URL}/rest/v1/faqs?select=*&order=sort_order.asc,created_at.asc`,
    {
      headers: {
