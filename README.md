...
- 장비 목록·실증 예시·이용 가이드·공간 단가는 `index.html`의 `SHEET_CSV_URLS`에 적힌 구글 시트(웹에 게시 CSV)에서 페이지를 열 때마다 불러옵니다. 시트 첫 행의 헤더 이름으로 용도를 판별하므로 헤더명은 바꾸지 않습니다. 장비 시트 열: `장비명, 건물, 공간, 카테고리, 연도, 수량, 가로(mm), 세로(mm), 높이(mm), 제품설명, 기본 사용 절차`(절차는 한 셀에 줄바꿈으로 단계 구분; 비어 있으면 카테고리 가이드 탭의 절차 사용). 시트를 고친 뒤 사이트에 반영되지 않으면 파일 > 공유 > 웹에 게시에서 "변경 시 자동으로 다시 게시" 체크 여부와 게시된 탭이 맞는지 확인합니다.
